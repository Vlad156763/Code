...
namespace data_struct_b_tree {
	...
	template <typename T> void b_tree<T>::add_key(const T& key) {
		...
		if (!node_must_have_new_key->leaf) { // якщо батько не листок 
...
void Fhome_Btree(dl_list<const char*>& way, bool& main_home_exit) {
...
static void Fhome_testTree(b_tree<int>& qa, dl_list<const char*>& way) {
	...
	cout << "\033[38;2;100;100;255m";
	for (int i = 0; i < way.get_size(); i++) cout << way[i];
	cout << "\033[0m" << endl << "Наповнення данними..." << endl << "Тестове дерево наповнене!" << endl;
